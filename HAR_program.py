...
TRAIN_PATH = '.\KTH Dataset\Train_data'
TEST_PATH = '.\KTH Dataset\Test_data'
...
class HARModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 1, VIDEO_HEIGHT, VIDEO_WIDTH)
        x = self.conv(x)
        x = self.pool(x)

        #batch_first=True时，lstm的imput格式为（batch_size,len,channels)
        x = x.view(4, MAX_FRAMENUM, 400)         #(batch_size, len ,channel)
...
if __name__ == '__main__':
    ...
    for epoch in range(train_epoch):
        ...
        print('[%d , %d] loss: %.5f' % (epoch + 1, train_epoch, running_loss / len(train_data)))
            ...
            with torch.no_grad():
                for input_test, label_test in test_loader:
                    ...
                    for i in range(len(label_test)):
                        if output_test.argmax(1)[i] == label_test[i]:
                            test_num_total += 1
                            test_num_correct += 1
                        else:
                            test_num_total += 1
